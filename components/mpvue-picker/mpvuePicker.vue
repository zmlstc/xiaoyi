    ...
    <view class="mpvue-picker">
        ...
        <view class="mpvue-picker-content " :class="{'mpvue-picker-view-show':showPicker}">
            ...
            <picker-view indicator-style="height: 40px;" class="mpvue-picker-view" :value="pickerValue" @change="pickerChange" v-if="mode==='timeSelector'">
                <block>
                    <picker-view-column>
                        <view class="picker-item" v-for="(item,index) in pickerValueHour" :key="index">{{item.name}}</view>
                    </picker-view-column>
                    <picker-view-column>
                        <view class="picker-item" v-for="(item,index) in pickerValueMinute" :key="index">{{item.name}}</view>
                    </picker-view-column>
                </block>
            </picker-view>
            <!-- 多列选择 -->
            <picker-view indicator-style="height: 40px;" class="mpvue-picker-view" :value="pickerValue" @change="pickerChange" v-if="mode==='multiSelector'">
                <block v-for="(n,index) in pickerValueMulArray.length" :key="index">
                    <picker-view-column>
                        <view class="picker-item" v-for="(item,index1) in pickerValueMulArray[n]" :key="index1">{{item.name}}</view>
                    </picker-view-column>
                </block>
            </picker-view>
            <!-- 二级联动 -->
        ...
        methods: {
            ...
            _getPickerLabelAndValue(value, mode) {
             ...
             /*   if (mode === 'selector') {
                    ...
                    pickerLable =
                        this.deepLength === 2 ?
                        `${this.pickerValueMulTwoOne[value[0]].name}-${this.pickerValueMulTwoTwo[value[1]].name}` :
                        `${this.pickerValueMulThreeOne[value[0]].name}-${this.pickerValueMulThreeTwo[value[1]].name}-${this.pickerValueMulThreeThree[value[2]].name}`;
                    if (this.deepLength === 2) {
               ...
               /* } */
				...
				rate =
					   this.deepLength === 2 ?
					   `${this.pickerValueMulTwoTwo[value[1]].rate}` :
