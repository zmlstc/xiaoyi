        ...
        <view class="content">
          <view class="section" id="current">
            ...
            <view class="city-list">
              <view class="city-item">{{value}}</view>
            </view>
          </view>
          <view class="section" id="hot" v-if="hotCitys.length">
            <view class="city-title">热门城市</view>
            <view class="city-list">
              <view class="city-item" :class="{active: current === city.name}" v-for="(city, i) in hotCitys" :key="i" @click="onSelect(city)">{{city.name}}</view>
            </view>
          </view>
          <view class="section" :id="item.letter" v-for="item in citys" :key="item.letter">
            <view class="letter">{{item.letter}}</view>
            <view class="city-list">
              <view class="city-item" :class="{active: current === city.name}" v-for="(city,itemIndex) in item.list" :key="itemIndex" @click="onSelect(city)">{{city.name}}</view>
            </view>
          </view>
        </view>
      </scroll-view>
...
<script>
//import Citys from '../../static/temp/citys.js'
  export default {
    props: {
      hotCitys: {
        type: Array,
