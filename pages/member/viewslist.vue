<template>
	<view class="mt10">
		<view class="show-box row mb2" v-for="(item ,index) in list":key="index" @tap="toPage(item.store_id)">
			<!-- 删除按钮  -->
        ...
        methods: {   
            async loadData() {  
                ...
                if (!this.hasLogin) {  
					...
					_this.$Request.post('store_browse/browselist',{curpage:_this.curpage}).then(res => {
						if (res.code == 200) {
							console.log(res);
			...
			deleteOrder(id){
				...
				_this.$Request.post('store_browse/browsedel',{store_id:id}).then(res => {
					if (res.code == 200) {
						console.log(res);
