<template>
	<view class="content">
		...
		<view class="share-bar b-line">
			<view class="item">共<text>{{num}}</text>家</view>
			<view class="item2">
				<view class="sc-bar">
		...
		<view class="share-list b-line"  v-for="(item ,index) in list":key="index">
			<view class="item">
				...
				<text class="ml10 fl">{{item.store_name}}</text>
			</view>
    ...
    export default { 
        ...
        methods: {  
			loadInvietNum(){
				...
				_this.$Request.post('member_invite/sinvitenum',postData).then(res => {
					if (res.code == 200) {
						console.log(res);
            ...
            async loadData() {  
                ...
                if (!this.hasLogin) {  
					...
					_this.$Request.post('member_invite/storelist',postData).then(res => {
						if (res.code == 200) {
							console.log(res);
