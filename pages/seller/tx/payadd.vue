<template>
	<view>
		<view class="cash-header mb20">
			<view class="paymode wxpay"></view>
			<view class="accname mt10" v-if="id>0"><view class="point">绑定</view>{{bindacctxt}}</view>
			<view class="accname mt10" v-if="id==0">添加微信提现账号</view>
		</view>		

		<view class="cash-add bf mt5"  v-if="current =='alipay'">
		...
		<view class="cash-add bf mt5" v-if="current =='wxpay'">
			<view class="item f-r">账号名称</view>
			<view class="item2 ml20">
				<open-data type="userNickName" v-model="wxnick"></open-data></view>
		</view>
		<view class="cash-add bf mt5">
			<view class="item f-r">真实姓名</view>
			<view class="item2 ml20">
				<input placeholder="微信支付绑定的真实姓名" placeholder-class="pl-class" v-model="uname" maxlength="50" /></view>
		</view>
		<view class="cash-add bf mt5">
		...
		<button class="btn mt40"  open-type='getUserInfo'  @getuserinfo="formSubmit" :disabled="logining" >确认提交</button>
	</view>
</template>
<script> 
    import {  
        ...
        methods: {  
            async loadData() { 				
                ...
                if (!this.sellerHasLogin) {  
					...
					if(_this.id>0){
						_this.$Request.post('seller_fund/gettxwayone',{id:_this.id}).then(res => {
							if (res.code == 200) {
								console.log(res);
			...
			formSubmit: function (e) {
				...
				_this.$Request.post('connect/check_sms',{phone:_this.phoneNumber,yzcode:_this.smscode,type:5}).then(res => {
					if (res.code == 200) {
						console.log(res);
						let postData={phone:_this.phoneNumber,yzcode:_this.smscode,type:1,openid:_this.openid,id:_this.id,
