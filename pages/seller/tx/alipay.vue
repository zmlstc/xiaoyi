<template>
	<view>
		<view class="cash-header mb20">
			<view class="paymode alipay"></view>
			<view class="accname mt10" v-if="id>0"><view class="point">绑定</view>{{bindacctxt}}</view>
			<view class="accname mt10" v-if="id==0">添加支付宝提现账号</view>
		</view>
		...
		<view class="cash-add bf mt5">
			...
			<view class="item2 ml20">
				<input placeholder="输入你的真实姓名" placeholder-class="pl-class" v-model="uname" maxlength="50" /></view>
		</view>
		<view class="cash-add bf mt5">
...
<script> 
    ...
    export default {  
		...
		onLoad(e) {
			 console.log('页面加载'); console.log(e);
			 this.id=e.id;
			this.loadData();
		},
        methods: {  
                ...
                if (!this.sellerHasLogin) {  
					...
					if(_this.id>0){
						_this.$Request.post('seller_fund/gettxwayone',{id:_this.id}).then(res => {
							if (res.code == 200) {
								console.log(res);
								if(res.datas.notxway==0 && res.datas.txway.bill_type_code=='alipay'){
			...
			formSubmit: function (e) {
				...
				_this.$Request.post('connect/check_sms',{phone:_this.phoneNumber,yzcode:_this.smscode,type:5}).then(res => {
					if (res.code == 200) {
						console.log(res);
						let postData={phone:_this.phoneNumber,yzcode:_this.smscode,type:2,openid:_this.openid,id:_this.id,
								bill_user_name:_this.uname,bill_type_number:_this.unumber,bill_type_code:'alipay'};
