	...
	<view>
		...
		<view class="uni-tab-bar">
			<scroll-view id="tab-bar" class="uni-swiper-tab" scroll-x >
				<view v-for="(tab,index) in tabClasslist" :key="tab.sc_id" :class="['swiper-tab-list',tabIndex==tab.sc_id ? 'active' : '']" :id="tab.sc_id"
		...
		<view class="guesslike-section">
			<view class="goods-list" v-if="storelist.length > 0">
				<view class="coupon-page s-row" v-for="(item,index) in storelist" :key="index" @tap="toStore(item.store_id)">
					...
					<view class="content">
						<view class="title s-row"><text>{{item.store_name}}</text><text class="distance"> {{item.distance}}KM</text></view>
						<view class="num s-row">
							<view class="start"><uni-rate disabled="true" size="14" margin="5" :value="item.scores"></uni-rate></view>
		...
		<view class="bottom-nav">
			<view class="mian">
				<view class="index mc"></view>
				<view class="title"><text>小易</text></view>
			</view>
			<view class="mian">
				<view class="scan mc"></view>
				<view class="title">扫码</view>
			</view>
			<view class="mian">
				<view class="my mc"></view>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			var _this =this;
			_this.tabClasslist = [{sc_name: '首页',sc_id: '-1'},{sc_name: '全部',sc_id: '0'}];
		...
		methods: {
			async loadData() {
				...
				_this.$Request.post('shop/shop_list',{cateid:_this.cateid,page:10,curpage:_this.page,lat:_this.mapinfo.lat,lng:_this.mapinfo.lng,adcode:_this.mapinfo.adcode,city_id:_this.mapinfo.city_id}).then(res => {
					if (res.code == 200) {
						console.log(res);
						_this.storelist = res.datas.store_list;
						/* this.bannerImg = res.datas.aplist;
						if(res.datas.sc_list){
							for (let i = 0; i < res.datas.sc_list.length;i++) {
								this.tabClasslist.push(res.datas.sc_list[i]);
							}
						} */
						console.log(_this.tabClasslist);
					}
	...
	.seckill-section{
		...
		.s-header{
			...
			.title{
				...
				color: #000;
				margin-right: 20upx;
				border-bottom: 0.5vw solid #01847F;
			}
			.other{
